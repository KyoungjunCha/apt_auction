...
public class CommentController {
    ...
    public String createAnswerComment(@PathVariable("id") Integer id, @Valid CommentForm commentForm,
            BindingResult bindingResult, Principal principal) {
        Optional<Answer> question = Optional.ofNullable(this.answerService.getAnswer(id));
        Optional<SiteUser> user = Optional.of(this.userService.getUser(principal.getName()));
        if (question.isPresent() && user.isPresent()) {
            if (bindingResult.hasErrors()) {
                return "comment_form";
