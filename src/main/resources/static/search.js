...
// 검색 결과 목록과 마커를 표출하는 함수입니다
function displayPlaces(auctionMasters) {
    ...
    for (var i = 0; i < places.length; i++) {
        ...
        var marker = addMarker(placePosition, i),
            itemEl = getListItem(i, auctionMasters[i]); // 검색 결과 항목 Element를 생성합니다
        ...
        // 해당 장소에 인포윈도우에 장소명을 표시합니다
        // mouseout 했을 때는 인포윈도우를 닫습니다
        (function (marker, auctionMasters) {
            kakao.maps.event.addListener(marker, 'mouseover', function () {
                displayInfowindow(marker, auctionMasters);
            });
        ...
        })(marker, auctionMasters[i].auctionKey);
